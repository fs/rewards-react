export default {
  data: [
    {
      id: '289',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning for move',
        points: 1,
        'total-points': 1,
        'created-at': '2019-05-16T06:07:25.102Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '284',
      type: 'bonuses',
      attributes: {
        text: '#test  +1  @almaz.karimullin ',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-30T10:45:31.860Z',
      },
      relationships: {
        sender: {
          data: {
            id: '63',
            type: 'users',
          },
        },
        tags: {
          data: [
            {
              id: '9',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [
            {
              id: '355',
              type: 'comments',
            },
          ],
        },
        receivers: {
          data: [
            {
              id: '50',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '283',
      type: 'bonuses',
      attributes: {
        text: '#be-curious-never-stop-learning @albert.fazullin +5 test',
        points: 5,
        'total-points': 5,
        'created-at': '2019-04-30T08:08:48.835Z',
      },
      relationships: {
        sender: {
          data: {
            id: '39',
            type: 'users',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '51',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '282',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning for move',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-29T14:45:01.136Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '281',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning for move',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-29T14:09:28.921Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '280',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning for excercise',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-29T14:09:03.831Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '279',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning for stand',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-29T13:41:07.396Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '277',
      type: 'bonuses',
      attributes: {
        text: '+5 @albert.fazullin #be-curious-never-stop-learning test',
        points: 5,
        'total-points': 5,
        'created-at': '2019-04-24T10:23:19.956Z',
      },
      relationships: {
        sender: {
          data: {
            id: '4',
            type: 'users',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '51',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '276',
      type: 'bonuses',
      attributes: {
        text: '+1 to @ivan.ananev #be-curious-never-stop-learning Hey Ho!!!',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-18T11:04:51.123Z',
      },
      relationships: {
        sender: {
          data: {
            id: '2',
            type: 'bots',
          },
        },
        tags: {
          data: [
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '77',
              type: 'users',
            },
          ],
        },
      },
    },
    {
      id: '274',
      type: 'bonuses',
      attributes: {
        text: '+1 @olga.krasnova @albert.fazullin @leyla.khamidullina #be-curious-never-stop-learning test react rewards #create-awesomeness',
        points: 1,
        'total-points': 1,
        'created-at': '2019-04-17T16:06:47.974Z',
      },
      relationships: {
        sender: {
          data: {
            id: '39',
            type: 'users',
          },
        },
        tags: {
          data: [
            {
              id: '2',
              type: 'tags',
            },
            {
              id: '6',
              type: 'tags',
            },
          ],
        },
        comments: {
          data: [],
        },
        receivers: {
          data: [
            {
              id: '34',
              type: 'users',
            },
            {
              id: '51',
              type: 'users',
            },
          ],
        },
      },
    },
  ],
  included: [
    {
      id: '2',
      type: 'bots',
      attributes: {
        name: 'activity',
      },
    },
    {
      id: '6',
      type: 'tags',
      attributes: {
        label: 'be-curious-never-stop-learning',
      },
    },
    {
      id: '77',
      type: 'users',
      attributes: {
        email: 'ivan.ananev@flatstack.com',
        'full-name': 'Ivan Ananev',
        username: 'ivan.ananev',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 6,
        'allowance-balance': 500,
      },
    },
    {
      id: '63',
      type: 'users',
      attributes: {
        email: 'mariya.valeeva@flatstack.com',
        'full-name': 'Mariya Valeeva',
        username: 'mariya-valeeva',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 8479,
        'allowance-balance': 828,
      },
    },
    {
      id: '9',
      type: 'tags',
      attributes: {
        label: 'test',
      },
    },
    {
      id: '355',
      type: 'comments',
      attributes: {
        'created-at': '2019-05-13T07:20:38.539Z',
        text: 'test comment',
      },
      relationships: {
        bonus: {
          data: {
            id: '284',
            type: 'bonuses',
          },
        },
        sender: {
          data: {
            id: '39',
            type: 'users',
          },
        },
        tags: {
          data: [],
        },
      },
    },
    {
      id: '39',
      type: 'users',
      attributes: {
        email: 'nadezhda.kharchuk@flatstack.com',
        'full-name': 'Nadezhda Kharchuk',
        username: 'nadezhda.kharchuk',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 78,
        'allowance-balance': 491,
      },
    },
    {
      id: '50',
      type: 'users',
      attributes: {
        email: 'almaz.karimullin@flatstack.com',
        'full-name': 'Almaz Karimullin',
        username: 'almaz.karimullin',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 176,
        'allowance-balance': 500,
      },
    },
    {
      id: '51',
      type: 'users',
      attributes: {
        email: 'albert.fazullin@flatstack.com',
        'full-name': 'Albert Fazullin',
        username: 'albert.fazullin',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 57,
        'allowance-balance': 500,
      },
    },
    {
      id: '4',
      type: 'users',
      attributes: {
        email: 'timur.vafin@flatstack.com',
        'full-name': 'Timur Vafin',
        username: 'timur.vafin',
        'profile-image-avatar-url': 'https://d14n4flg6h47l7.cloudfront.net/store/user/4/profile_image/avatar-e648f6ad68c82a09355e7b3928d8f010.png?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAIJWLCQQNSWJW7QQA%2F20190515%2Feu-central-1%2Fs3%2Faws4_request&X-Amz-Date=20190515T073433Z&X-Amz-Expires=900&X-Amz-SignedHeaders=host&X-Amz-Signature=ffc60d53a480b2483bf456946eb68d3991a10421ed4fba8ef165633e379f0282',
        'bonus-balance': 4934,
        'allowance-balance': 495,
      },
    },
    {
      id: '2',
      type: 'tags',
      attributes: {
        label: 'create-awesomeness',
      },
    },
    {
      id: '34',
      type: 'users',
      attributes: {
        email: 'leyla.khamidullina@flatstack.com',
        'full-name': 'Leyla Khamidullina',
        username: 'leyla.khamidullina',
        'profile-image-avatar-url': 'http://doypx5fd26upp.cloudfront.net/assets/default-user-profile_image-b50dbea6a3e28a1ae62538d4b0115b18558fac3ff7dbbee7da800b5216f97070.png',
        'bonus-balance': 50079,
        'allowance-balance': 500,
      },
    },
  ],
  links: {
    self: 'http://rewards-staging.flatstack.com/api/v1/user/bonuses?page%5Bnumber%5D=1&page%5Bsize%5D=10',
    first: 'http://rewards-staging.flatstack.com/api/v1/user/bonuses?page%5Bnumber%5D=1&page%5Bsize%5D=10',
    prev: null,
    next: 'http://rewards-staging.flatstack.com/api/v1/user/bonuses?page%5Bnumber%5D=2&page%5Bsize%5D=10',
    last: 'http://rewards-staging.flatstack.com/api/v1/user/bonuses?page%5Bnumber%5D=22&page%5Bsize%5D=10',
  },
};
